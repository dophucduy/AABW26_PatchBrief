import type { MappingRow, PatchReport, RequiredFile } from './types';
...
export const demoReport: PatchReport = {
  ...
  alignment: {
    ...
    patterns: [
      ...
      { id: 'perception', title: 'Perception versus data', description: 'Community reports weak; win rate near meta average', confidence: 'medium' },
    ],
  },
  risks: [
    { id: 'stakeholder_conflict', severity: 'high', title: 'Casual players favor buff; ranked meta stable', evidence: ['Win rate high bracket: 49% near average', "Community theme 'feels weak': 340 mentions"] },
  ...
  validation_plan: [
    'Playtest Ironclad in low versus high bracket after the micro-adjustment',
    'Monitor pick-rate split for one week post-patch',
    'Survey: fun to play versus feels weak',
  ],
...
export const mappingRows: MappingRow[] = [
  ...
  { source: 'gameTime', sample: '45.1', target: 'timestamp', confidence: 0.78, kind: 'Field' },
