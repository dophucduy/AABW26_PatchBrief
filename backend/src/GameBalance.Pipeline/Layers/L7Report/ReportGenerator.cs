...
public sealed class ReportGenerator
{
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        {
            _logger?.LogWarning("L7 report using template fallback: Llm:ApiKey is not configured");
            return TemplateReportBuilder.Build(impact, risks, context);
        }

        if (httpClient is null)
        {
            _logger?.LogWarning("L7 report using template fallback: HttpClient was not provided");
            return TemplateReportBuilder.Build(impact, risks, context);
        }
        ...
        try
        {
            ...
            {
                _logger?.LogInformation("L7 report generated by LLM ({Provider})", options.Provider);
                return llmReport;
            }

            _logger?.LogWarning(
                "L7 report using template fallback: LLM returned an empty or unparseable response");
            return TemplateReportBuilder.Build(impact, risks, context);
        }
        catch (Exception exception)
        {
            _logger?.LogWarning(
                exception,
                "L7 report using template fallback: LLM call failed ({Provider})",
                options.Provider);
            return TemplateReportBuilder.Build(impact, risks, context);
        }
    }

    private async Task<InsightReport?> CallOpenAiAsync(
        MetricResult metrics,
        ContextBundle context,
        ImpactResult impact,
        RiskResult risks,
        ...
        CancellationToken cancellationToken)
    {
        ...
        if (!response.IsSuccessStatusCode)
        {
            ...
            _logger?.LogWarning(
                "OpenAI request failed with status {StatusCode}: {Body}",
    ...
    private async Task<InsightReport?> CallGeminiAsync(
        MetricResult metrics,
        ContextBundle context,
        ImpactResult impact,
        RiskResult risks,
    ...
    private InsightReport? ParseLlmReport(string? content, ImpactResult impact, RiskResult risks)
    {
        ...
        try
        {
            payload = JsonSerializer.Deserialize<LlmReportPayload>(content, ParseOptions);
        }
        catch (JsonException exception)
        {
            _logger?.LogWarning(exception, "Failed to parse LLM JSON response");
            return null;
        }

        if (payload is null || string.IsNullOrWhiteSpace(payload.ExecutiveSummary))
        {
            _logger?.LogWarning(
                "LLM JSON response missing executive_summary. Raw response: {Content}",
                Truncate(content, 500));
            return null;
        }

        string markdown = string.IsNullOrWhiteSpace(payload.ReportMarkdown)
